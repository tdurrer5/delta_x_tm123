...
void MotionClass::G0(float xPos, float yPos, float zPos, float ewPos)
{
	...
	if (!Tool.CheckingDesiredPoint(pointBuffer))  {
	    return;
	}
...
void MotionClass::G2(float i, float j, float xPos, float yPos, float wPos) {
	...
	if (!CircleInterpolation(i, j, false)) {
	    return;
	}
...
void MotionClass::G3(float i, float j, float xPos, float yPos, float wPos)
{
	...
	if (!CircleInterpolation(i, j, true)) {
	    return;
	}
...
void MotionClass::G5(float i, float j, float p, float q, float x, float y, float wPos) {
	...
	if (!Bezier4PointInterpolation(p1, p2)) {
	    return;
	}
...
void MotionClass::G28() {
	...
	MultiServo.AddAngle(AXIS_4, angle);
...
bool MotionClass::LinearInterpolation()
{
	...
	{
		const float tbuffer = (float)i / NumberSegment;
...
bool MotionClass::CircleInterpolation(float i, float j, bool clockwise)
{
	...
	if (angular_travel == 0 &&
	        abs(Data.CurrentPoint.X - Data.DesiredPoint.X) < 0.4 &&
	        abs(Data.CurrentPoint.Y - Data.DesiredPoint.Y) < 0.4)
	{
		if (clockwise)
		{
			angular_travel = -2.0 * PI;
	...
	if (!DeltaKinematics.InverseKinematicsCalculations(Data.DesiredPoint, currentAngle)) {
	    return false;
	}

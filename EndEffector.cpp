...
{
	pinMode(VACCUM_PIN, OUTPUT);                //10  PA7

	pinMode(SPINDLE_LASER_ENABLE_PIN, OUTPUT);  // 10 PA7
	pinMode(SPINDLE_LASER_PWM_PIN, OUTPUT);     // 5  PE4

	pinMode(CUSTOM_PWM_PIN, OUTPUT);    // 4    PB1
	pinMode(CUSTOM_DIR_PIN, OUTPUT);    // 16   ?

	pinMode(EXTRUSDER_PULSE_PIN, OUTPUT);       // 26 PD3
	pinMode(EXTRUSDER_DIRECTION_PIN, OUTPUT);   // 28 PE2
	pinMode(EXTRUSDER_ENABLE_PIN, OUTPUT);      // 24 PD1
...
void EndEffectorClass::ResetEndEffectorOutput()
{
	...
	{
		MultiServo.servos[1].servo.attach(CLIP_SERVO_PIN);
	}
